...
#var url: String = "http://eddge-sp-ai1:11434/v1/chat/completions"
var url: String = "http://eddge-sp-ai1:11434/api/chat"
...
func dialogue_request(user_dialogue, content, model):
	...
	var body = JSON.new().stringify({"prompt": "hello", "model": model})
...
func _on_request_completed(result, response_code, headers, body):
	...
	if error != OK:
		print(error)
		display_error_message("An error 2 occurred while parsing the response. \n*Make sure the model [b]'%s' is running[/b] in the background." % model)
		return
